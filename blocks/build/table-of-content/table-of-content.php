...
class TableOfContent {
	...
	public static function add_toc( $content = '' ) {
		...
		if ( $items ) {
			$toc .= '<div class="card bg-secondary text-primary shadow-regular">';
			$toc .= '<h3 class="mx-16" id="content"><i class="fa-solid fa-table-list"></i> Contents</h3>';
            $toc .= '<ul class="list-group list-group-flush">';
			foreach ( $items as $item ) {
				$toc .= '<li class="list-group-item list-group-item-action list-group-item-secondary"><a href="#' . sanitize_title_with_dashes( $item[2] ) . '">' . $item[2] . '</a></li>';
			}
			$toc .= '</ul>';
			$toc .= '</div>';
		}
	...
	private static function add_ids_and_jumpto_links( $tag, $content ) {
		...
		foreach ( $items as $item ) {
			...
			$replacement   .= sprintf( '<%1$s id="%2$s"><a href="#%2$s" class="anchor link-info">%3$s <i class="fa-solid fs-12 fa-link"></i></a></%1$s>', $tag, $id, $item[2] );
