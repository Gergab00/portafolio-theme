jQuery( document ).ready( function ( $ ) {
	...
	$( '.customize-control-sortable-repeater-add' ).click( function ( event ) {
		...
		skyrocketAppendRow(
			$( this ).parent(),
			'',
			$( '.repeater-input' ).last().attr( 'placeholder' )
		);
		skyrocketGetAllInputs( $( this ).parent() );
	} );
	...
	function skyrocketAppendRow(
		...
		var newRow =
			...
			defaultValue +
			'" class="repeater-input" placeholder="' +
			placeholder +
			'" /><span class="dashicons dashicons-sort"></span><a class="customize-control-sortable-repeater-delete" href="#"><span class="dashicons dashicons-no-alt"></span></a></div>';
	...
	$( '.slider-custom-control' ).each( function () {
		...
		var sliderStepValue = parseFloat(
			newSlider.attr( 'slider-step-value' )
		);
	...
	function skyrocketGetAllImageCheckboxes( $element ) {
		...
		// Important! Make sure to trigger change event so Customizer knows it has to save the field
		$element
			.find( '.customize-control-multi-image-checkbox' )
			.val( inputValues )
			.trigger( 'change' );
	}
	...
	$( '.google-fonts-list' ).on( 'change', function () {
		...
		var elementBoldWeight = $( this )
			.parent()
			.parent()
			.find( '.google-fonts-boldweight-style' );
		...
		var bodyfontcontrol =
			_wpCustomizeSettings.controls[ customizerControlName ];

		// Find the index of the selected font
		var indexes = $.map(
			bodyfontcontrol.skyrocketfontslist,
			function ( obj, index ) {
		...
		$.each(
			bodyfontcontrol.skyrocketfontslist[ index ].variants,
			function ( val, text ) {
		...
		if ( elementItalicWeightCount == 0 ) {
			elementItalicWeight.append(
				$( '<option></option>' )
					.val( '' )
					.html( 'Not Available for this font' )
			);
	...
	function skyrocketGetAllSelects( $element ) {
		var selectedFont = {
			...
			regularweight: $element
				.find( '.google-fonts-regularweight-style' )
				.val(),
			italicweight: $element
				.find( '.google-fonts-italicweight-style' )
				.val(),
	...
	$( '.customize-control-tinymce-editor' ).each( function () {
		...
		var tinyMCEToolbar1String =
			_wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ]
				.skyrockettinymcetoolbar1;
		var tinyMCEToolbar2String =
			_wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ]
				.skyrockettinymcetoolbar2;
		var tinyMCEMediaButtons =
			_wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ]
				.skyrocketmediabuttons;
