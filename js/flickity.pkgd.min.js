...
! ( function ( t, e ) {
					...
					? ( function ( t, e, n ) {
							...
							return (
								t.each( function ( t, h ) {
									...
									if ( ! a )
										return void s(
											`${ i } not initialized. Cannot call method ${ r }`
										);
									let c = a[ e ];
									if ( ! c || '_' == e.charAt( 0 ) )
										return void s(
											`${ r } is not a valid method`
										);
									let d = c.apply( a, n );
									l = void 0 === l ? d : l;
								} ),
	...
	( function ( t, e ) {
		...
		return function ( i ) {
			...
			let p = t( s.height );
			return (
				! 1 !== p && ( n.height = p + ( o ? 0 : r + d ) ),
				( n.innerWidth = n.width - ( l + c ) ),
				( n.innerHeight = n.height - ( r + d ) ),
	...
	( function ( t, e ) {
		...
		( o.handleEvent = function ( t ) {
			...
			'ontouchstart' in t
				? ( ( s = 'touchstart' ),
				  ( n = [ 'touchmove', 'touchend', 'touchcancel' ] ) )
				: t.PointerEvent
				? ( ( s = 'pointerdown' ),
	...
	( function ( t, e ) {
		...
		let e = {
			...
			positionSlider: function () {
				...
				this.isWrapping &&
					( ( e =
						t.modulo( e, this.slideableWidth ) -
						this.slideableWidth ),
			...
			_unshiftCells: function ( t ) {
				t &&
					t.length &&
					t.forEach( ( t ) => this._wrapShiftCell( t, 0 ) );
			},
	...
	( function ( t, e ) {
		...
		function ( t, e, i, s, n, o, l ) {
			const { getComputedStyle: r, console: h } = t;
			let { jQuery: a } = t,
				c = 0,
				d = {};
			function u( t, e ) {
			...
			Object.assign( p, e.prototype ),
				...
				( p.activate = function () {
					...
					( this.isActive = ! 0 ),
						...
						this.getSize();
					let t = this._filterFindCellElements(
				...
				( p._makeCells = function ( t ) {
					return this._filterFindCellElements( t ).map(
						( t ) => new n( t )
					);
				} ),
			...
			( p.emitEvent = function ( t, e ) {
				...
				( p.watchCSS = function () {
					...
					r( this.element, ':after' ).content.includes( 'flickity' )
						? this.activate()
						: this.deactivate();
				} ),
				...
				( u.keyboardHandlers = {
					ArrowLeft: function () {
						this.uiChange(),
							this[
								this.options.rightToLeft ? 'next' : 'previous'
							]();
					},
					ArrowRight: function () {
						this.uiChange(),
							this[
								this.options.rightToLeft ? 'previous' : 'next'
							]();
					},
				} ),
				( p.focus = function () {
		...
		return (
			...
			( n.handleDragEnd = function () {
				...
				if ( t && ! this.isWrapping ) {
					...
					this.isFreeScrolling =
						-t > this.slides[ 0 ].target &&
						-t < this.getLastSlide().target;
				} else
					t ||
		...
		return (
			( i.insert = function ( t, e ) {
				...
				let n = ( function ( t ) {
						...
						return (
							t.forEach( ( t ) => e.appendChild( t.element ) ), e
						);
					} )( i ),
					o = e === s;
		...
		return (
			( l.lazyLoad = function () {
				...
				this.getAdjacentCellElements( e )
					...
					.flat()
					.forEach(
						( t ) => new h( t, this.handleLazyLoadComplete )
					);
			} ),
			( l.onLazyLoadComplete = function ( t, e ) {
				let i = this.getParentCell( t ),
					s = i && i.element;
				this.cellSizeChange( s ),
					this.dispatchEvent( 'lazyLoad', e, s );
			} ),
			( h.prototype.handleEvent = e.handleEvent ),
			( h.prototype.load = function () {
				this.img.addEventListener( 'load', this ),
					this.img.addEventListener( 'error', this );
				let t =
						this.img.getAttribute( i ) ||
						this.img.getAttribute( s ),
