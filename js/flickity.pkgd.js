...
( function ( window, factory ) {
	...
	function factory( window, EvEmitter ) {
		...
		// inherit EvEmitter
		let proto = ( Unidragger.prototype = Object.create(
			EvEmitter.prototype
		) );
		...
		proto.pointerDown = function ( event, pointer ) {
			...
			let isOkay =
				! this.isPointerDown && ! event.button && isOkayElement;
			if ( ! isOkay ) return;
		...
		proto.pointerMove = function ( event, pointer ) {
			...
			let isDragStarting =
				! this.isDragging && this.hasDragStarted( moveVector );
...
( function ( window, factory ) {
	...
	proto.getPositionValue = function ( position ) {
		if ( this.options.percentPosition ) {
			...
			return (
				Math.round( ( position / this.size.innerWidth ) * 10000 ) *
					0.01 +
				'%'
			);
		} else {
	...
	function factory(
		...
		proto._wrapSelect = function ( index ) {
			...
			if ( ! this.isDragSelect ) {
				...
				let backWrapDelta = Math.abs(
					wrapIndex + length - selectedIndex
				);
				let forewardWrapDelta = Math.abs(
					wrapIndex - length - selectedIndex
				);
		...
		proto.getCellSlideIndex = function ( cell ) {
			...
			let cellSlide = this.slides.find( ( slide ) =>
				slide.cells.includes( cell )
			);
		...
		proto.watchCSS = function () {
			...
			let afterContent = getComputedStyle(
				this.element,
				':after'
			).content;
		...
		proto.onkeydown = function ( event ) {
			...
			let isFocused = this.focusableElems.some(
				( elem ) => activeElement === elem
			);
		...
		Flickity.keyboardHandlers = {
			...
			ArrowRight: function () {
				this.uiChange();
				let rightMethod = this.options.rightToLeft
					? 'previous'
					: 'next';
				this[ rightMethod ]();
			},
	...
	function factory( window, Flickity, Unidragger, utils ) {
		...
		proto.handleDragMove = function ( event, pointer, moveVector ) {
			...
			if ( ! this.isWrapping ) {
				...
				dragX =
					dragX > originBound ? ( dragX + originBound ) * 0.5 : dragX;
		...
		proto.dragEndRestingSelect = function () {
			...
			let positiveResting = this._getClosestResting(
				restingX,
				distance,
				1
			);
			let negativeResting = this._getClosestResting(
				restingX,
				distance,
				-1
			);
...
( function ( window, factory ) {
    ...
    Z`;
		...
		proto.updatePrevNextButton = function ( button, disabledIndex ) {
			...
			let isDisabledFocused =
				! isEnabled && document.activeElement === button.element;
...
( function ( window, factory ) {
	...
	function factory( Flickity, utils ) {
		...
		PageDots.prototype.removeDots = function ( count ) {
			...
			let removeDots = this.dots.splice(
...
( function ( window, factory ) {
	...
	function factory( Flickity, utils ) {
		...
		LazyLoader.prototype.complete = function ( event, className ) {
			...
			this.img.removeEventListener( 'load', this );
			this.img.removeEventListener( 'error', this );
			let mediaElem = this.img.parentNode.matches( 'picture' )
				? this.img.parentNode
				: this.img;
