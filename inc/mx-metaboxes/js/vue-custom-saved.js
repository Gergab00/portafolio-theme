		...
		methods: {
			...
			push_element_data( _obj ) {
				...
				for ( const [ key, value ] of Object.entries( _obj ) ) {
					if (
						typeof this.block_data[ value.block_name ] !== 'object'
					) {
			...
			set_block_patern() {
				if ( Object.keys( this.block_patern ).length === 0 ) {
					for ( const [ key, value ] of Object.entries(
						...
						this.block_patern[ key ][ 'input_type' ] =
							value[ 'input_type' ];
	...
	let app = new Vue( {
		...
		methods: {
			...
			remove_element( el_data ) {
				...
				for ( const [ key, value ] of Object.entries(
					this.blocks[ el_data.block_name ]
				) ) {
			...
			save_data( data ) {
				...
				this.time_out = setTimeout( function () {
					...
					jQuery.post(
						mx_multibox_localize.ajax_url,
						data,
						function ( response ) {
							jQuery( '#' + _this.save_data_input_id ).val(
								response
							);
				...
				this.time_out_main_block = setTimeout( function () {
					...
					for ( const [ key, value ] of Object.entries(
						...
						for ( const [ _key, _value ] of Object.entries(
							...
							if ( key === _key ) {
								// enter the block
								Object.keys( _value ).forEach( function (
									val,
									index
								) {
									// enter the element
									for ( const [
										__key,
										__value,
									] of Object.entries( _value[ val ] ) ) {
										if (
											_this.blocks[ _key ][ val ][
												__key
											][ 'value' ] !==
											_value[ val ][ __key ][ 'value' ]
										) {
											_this.blocks[ _key ][ val ][
												__key
											][ 'value' ] =
			...
			save_data_into_output_block( data ) {
				...
				this.time_out_output_block = setTimeout( function () {
					...
					for ( const [ key, value ] of Object.entries( data ) ) {
						if (
							typeof _this.blocks_output_data[ key ] !== 'object'
						) {
							...
							for ( const [ _key, _value ] of Object.entries(
								...
								_this.blocks_output_data[ key ][ _key ] =
									_value; // create and fill in an element
							}
						} else {
							// enter the element
							for ( const [ _key, _value ] of Object.entries(
